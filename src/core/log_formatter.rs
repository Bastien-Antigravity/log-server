...
pub fn format_log_message(
    ...
    if !module.is_empty() { meta.push(format!("mod={}", module)); }
    if !path_name.is_empty() { meta.push(format!("path={}", path_name)); }
    if !process_id.is_empty() { meta.push(format!("pid={}", process_id)); }
    if !process_name.is_empty() { meta.push(format!("pname={}", process_name)); }
    if !thread_id.is_empty() { meta.push(format!("tid={}", thread_id)); }
    if !thread_name.is_empty() { meta.push(format!("tname={}", thread_name)); }
    if !service_name.is_empty() { meta.push(format!("svc={}", service_name)); }
...
mod tests {
    ...
    fn test_format_log_message_basic() {
        ...
        let formatted = format_log_message(
            timestamp, "host1", "logger1", "INFO", "mod1", "file1.rs", "func1", "10", 
            "Hello World", "", "", "", "", "", "", ""
        );
    ...
    fn test_format_log_message_with_metadata() {
        let formatted = format_log_message(
            "time", "host", "log", "DEBUG", "mod", "file", "func", "1", 
            "msg", "path", "123", "pname", "456", "tname", "svc", "stack"
        );
